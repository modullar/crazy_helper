...
module PrimaryHeaderNavigationHelper
  def nav_primary(client: false)
    ...
    guest_navigation(client: client)
  end
  ...
  def guest_navigation(client: false)
    return guest_default_navigation.insert(1, about_us_tab) if client
  ...
  def messages_block
    "Messages#{messages_count_span}"
  end

  def messages_count_span
    (messages_count.positive? ? unread_span(messages_count) : '')
  end

  def unread_span(_messages_count)
